...
<nav class="navbar navbar-expand-lg navbar-light bg-light elevation">
	      		...
	      		<ul class="navbar-nav me-auto mb-2 mb-lg-0">
					...
					<div class="dropdown">
					    ...
					    <ul class="dropdown-menu">
					    	...
					    	<li class="active"><a href="listar_fornecedor.php">Listar Fornecedores</a></li>
  ...
  <section id="listagem2">
      ...
      <?php
		...
		$result_usuarios = "SELECT CODFORNECEDOR, FORNECEDOR, DTCRIACAO FROM fornecedores LIMIT $inicio, $qnt_result_pg";
		$resultado_usuarios = mysqli_query($conn, $result_usuarios);
		while($row_usuario = mysqli_fetch_assoc($resultado_usuarios)){
			?>
			
			<tr>
				<td><b><?php echo $row_usuario['FORNECEDOR'] ?></b></td>
				<td><?php echo $row_usuario['CODFORNECEDOR'] ?></td>
				<td><a href='editar_usuario.php?id="<?php echo $row_usuario['CODFORNECEDOR'] ?>"'>Editar</a><br><hr></td>
			</tr>
		<?php
		}?>
	</table><?php
		...
		$max_links = 3;
		?>
		<a href='listar_fornecedor.php?pagina=1'>Primeira</a>
		<?php 
		for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++){
			if($pag_ant >= 1){
				?>
				<a href='listar_fornecedor.php?pagina=<?php echo $pag_ant ?>'><?php $pag_ant ?></a>
				<?php 
			}
		}
		?>	
		<a href='listar_fornecedor.php?pagina=<?php echo $pagina ?>'><?php echo $pagina ?></a>
		<?php
		for($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++){
			if($pag_dep <= $quantidade_pg){
				?>
				<a href='listar_fornecedor.php?pagina=<?php echo $pag_dep ?>'><?php echo $pag_dep ?></a>
				<?php 
			}
		}
		?>
		<a href='listar_fornecedor.php?pagina=<?php echo $quantidade_pg ?>'>Última</a>
  </section>
</body>
</html>
