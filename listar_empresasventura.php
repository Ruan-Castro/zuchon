...
<nav class="navbar navbar-expand-lg navbar-light bg-light elevation">
	      		...
	      		<ul class="navbar-nav me-auto mb-2 mb-lg-0">
					...
					<div class="dropdown">
					    ...
					    <ul class="dropdown-menu">
					    	...
					    	<li><a href="listar_empresasventura.php">Listar Fornecedores</a></li>
					    	<li><a href="cadastro_fornecedor.php">Cadastrar novo</a></li>
					    	<li class="divider"></li>
					    	<li class="dropdown-header">Empresas Ventura</li>
					    	<li class="active"><a href="listar_empresasventura.php">Listar existentes</a></li>
  ...
  <section id="listagem2">
      ...
      <?php
		...
		$result_usuarios = "SELECT CODEMPRESA, EMPRESA, DTCRIACAO FROM empresasventura LIMIT $inicio, $qnt_result_pg";
		$resultado_usuarios = mysqli_query($conn, $result_usuarios);
		while($row_usuario = mysqli_fetch_assoc($resultado_usuarios)){
			?>
			
			<tr>
				<td><b><?php echo $row_usuario['EMPRESA'] ?></b></td>
				<td><?php echo $row_usuario['CODEMPRESA'] ?></td>
				<td><a href='editar_usuario.php?id="<?php echo $row_usuario['CODEMPRESA'] ?>"'>Editar</a><br><hr></td>
			</tr>
		<?php
		}?>
	</table><?php
		//Paginção - Somar a quantidade de usuários
		$result_pg = "SELECT COUNT(CODEMPRESA) AS num_result FROM empresasventura";
		...
		$max_links = 3;
		?>
		<a href='listar_empresa.php?pagina=1'>Primeira</a>
		<?php 
		for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++){
			if($pag_ant >= 1){
				?>
				<a href='listar_empresasventura.php?pagina=<?php echo $pag_ant ?>'><?php $pag_ant ?></a>
				<?php 
			}
		}
		?>	
		<a href='listar_empresasventura.php?pagina=<?php echo $pagina ?>'><?php echo $pagina ?></a>
		<?php
		for($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++){
			if($pag_dep <= $quantidade_pg){
				?>
				<a href='listar_empresasventura.php?pagina=<?php echo $pag_dep ?>'><?php echo $pag_dep ?></a>
				<?php 
			}
		}
		?>
		<a href='listar_empresasventura.php?pagina=<?php echo $quantidade_pg ?>'>Última</a>
  </section>
</body>
</html>
